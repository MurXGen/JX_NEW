...
const k = (v, sym = "$") => {
  const a = Math.abs(v);
  const s = a >= 1000 ? `${sym}${fmt(a / 1000, 2)}k` : `${sym}${fmt(a, a < 10 ? 2 : 0)}`;
  return `${v < 0 ? "−" : "+"}${s}`;
};
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "3%", height, width: "100%" }}>
      {morphed.map((v, i) => {
        ...
        return (
          <Tip key={i} content={tips?.[i]} block style={{ flex: 1, height: "100%" }}>
            <div className="jx-tip--col" style={{ flex: 1, height: "100%", position: "relative" }}>
              <div
                style={{
                  position: "absolute", left: 0, right: 0, borderRadius: 4,
...
function AxisLabels({ labels }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", font: "var(--text-caption)", color: "var(--color-text-muted)", marginTop: 6 }}>
...
function Empty({ height }) {
  return (
    <div style={{ height, display: "flex", alignItems: "center", justifyContent: "center", font: "var(--text-small)", color: "var(--color-text-muted)" }}>
...
function CandleChart({ candles, height = 220, sym = "$" }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "stretch", gap: "0.6%", height, width: "100%" }}>
      {candles.map((c, i) => {
        const up = c.c >= c.o;
        ...
        return (
          ...
          >
            <motion.div
              ...
              transition={{ delay: Math.min(i * 0.025, 0.8), duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
              style={{ flex: 1, position: "relative", transformOrigin: "bottom", height: "100%" }}
            >
              {/* wick */}
              <span
                style={{
                  position: "absolute", left: "50%", transform: "translateX(-50%)",
                  top: y(c.h), height: Math.max(1, y(c.l) - y(c.h)),
                  width: 1.5, background: color, opacity: 0.8,
                }}
              />
              {/* body */}
              <span
...
const buildEquityCandles = (closed, startingBalance, tf) => {
  ...
  const labelOf = (d) => {
    const dt = new Date(d);
    if (tf === "1D" || tf === "1W") return dt.toLocaleDateString("en-GB", { day: "2-digit", month: "short" });
    if (tf === "1M") return dt.toLocaleDateString("en-GB", { month: "short", year: "2-digit" });
    return String(dt.getFullYear());
  };
  ...
  closed.forEach((t) => {
    ...
    if (!cur || cur.key !== k) {
      ...
      cur = { key: k, label: labelOf(t.closeTime), o: equity, h: equity, l: equity, c: equity };
    }
...
function MiniSeg({ items, value, onChange }) {
  return (
    <div className="jx-seg jx-seg--inline" style={{ padding: 3 }}>
      {items.map((it) => (
        <button key={it} className={`jx-seg__btn ${value === it ? "jx-seg__btn--active" : ""}`} style={{ padding: "5px 10px", font: "var(--text-caption)", fontWeight: 600 }} onClick={() => onChange(it)}>
          {it}
        </button>
      ))}
    </div>
...
const LABEL = { font: "var(--text-label)", letterSpacing: "0.6px", textTransform: "uppercase", color: "var(--color-text-muted)" };
const DONUT_COLORS = ["var(--yellow-300)", "var(--green-500)", "var(--red-500)", "var(--gray-400)", "#3b82f6", "#8b5cf6"];

/* range helpers — windows are anchored to the latest trade so sample
   data from any date still renders */
const DAYS = { "1D": 1, "7D": 7, "1W": 7, "21D": 21, "30D": 30, "1M": 30, "90D": 90, "3M": 90, "1Y": 365 };
...
export default function OverviewPanel({
  ...
  useEffect(() => {
    const read = () => setTarget(Number(localStorage.getItem("jx-monthly-target")) || 15000);
    read();
  ...
  const closed = useMemo(
    () => trades.filter((t) => t.closeTime).sort((a, b) => new Date(a.closeTime) - new Date(b.closeTime)),
    [trades],
  );
  ...
  const hero = useMemo(() => {
    ...
    const volume = list.reduce((s, t) => s + Math.abs((t.avgEntryPrice || t.entryPrice || 0) * (t.totalQuantity || 0)), 0);
    let run = 0;
    const spark = list.map((t) => (run += Number(t.pnl) || 0));
    const prev = closed.filter((t) => new Date(t.closeTime).getTime() < prevCutoff).reduce((s, t) => s + (Number(t.pnl) || 0), 0);
    const delta = prev !== 0 ? (pnl / Math.abs(prev)) * 100 : null;
    return { pnl, trades: list.length, winRate: list.length ? (wins / list.length) * 100 : 0, bestDay, volume, spark, delta };
  }, [closed, heroRange]);
  ...
  const S = useMemo(() => {
    ...
    const sd = daily.length > 1 ? Math.sqrt(daily.reduce((s, v) => s + (v - mean) ** 2, 0) / (daily.length - 1)) : 0;

    let streak = 0;
    for (let i = allPnls.length - 1; i >= 0 && allPnls[i] > 0; i--) streak++;
    let best = 0, cur = 0;
    allPnls.forEach((p) => { cur = p > 0 ? cur + 1 : 0; best = Math.max(best, cur); });

    const holds = win.filter((t) => t.openTime).map((t) => new Date(t.closeTime) - new Date(t.openTime));
    holds.sort((a, b) => a - b);
    const medHold = holds.length ? holds[Math.floor(holds.length / 2)] : null;
    const holdStr = medHold ? `${Math.floor(medHold / 3600000)}h ${Math.round((medHold % 3600000) / 60000)}m` : "—";
    ...
    win.forEach((t) => {
      ...
      alloc.set(s, (alloc.get(s) || 0) + Math.abs(((t.avgEntryPrice || t.entryPrice) || 1) * (t.totalQuantity || 0)));
    });
    const allocList = [...alloc.entries()].sort((a, b) => b[1] - a[1]).slice(0, 4);

    const volByDay = new Map();
    win.forEach((t) => {
      const d = new Date(t.closeTime).toDateString();
      volByDay.set(d, (volByDay.get(d) || 0) + Math.abs(((t.avgEntryPrice || t.entryPrice) || 1) * (t.totalQuantity || 0)) * (t.pnl >= 0 ? 1 : -1));
    });
    ...
    const stratList = [...byStrategy.entries()].sort((a, b) => b[1].pnl - a[1].pnl).slice(0, 5);

    const planned = win.filter((t) => t.rulesFollowed === true);
    const unplanned = win.filter((t) => t.rulesFollowed === false);
    const wr = (list) => (list.length ? (list.filter((t) => t.pnl > 0).length / list.length) * 100 : null);
    const confidences = win.map((t) => Number(t.confidence)).filter((c) => c > 0);
    const discipline = {
    ...
    return {
      ...
      streak, bestStreak: best, holdStr,
      daily, dailyLabels, symPnl, allocList,
      vols: [...volByDay.values()], volLabels,
      grossWin, grossLoss,
      monthPnl, stratList, discipline,
      winTrades: win,
    };
  ...
  const wrSeries = useMemo(() => {
    ...
    const bucketMs = wrRange === "1W" ? 864e5 : wrRange === "1M" ? 3 * 864e5 : wrRange === "3M" ? 7 * 864e5 : 30 * 864e5;
    ...
    return [...buckets.entries()].sort((a, b) => a[0] - b[0]).map(([, e]) => (e.w / e.n) * 100);
  }, [closed, wrRange]);
  ...
  const maxSym = S.symPnl.length ? Math.max(...S.symPnl.map(([, v]) => Math.abs(v)), 1) : 1;
  const allocTotal = S.allocList.reduce((s, [, v]) => s + v, 0) || 1;
  const totalVolume = S.allocList.reduce((s, [, v]) => s + v, 0);
  const maxStrat = S.stratList.length ? Math.max(...S.stratList.map(([, v]) => Math.abs(v.pnl)), 1) : 1;

  const kpis = [
    { label: "Net P&L (all-time)", value: <CountUp value={S.net} format={(v) => k(v, currencySymbol)} />, sub: usingDummy ? "sample journal" : "across this journal", up: S.net >= 0 },
    { label: "Total trades", value: <CountUp value={S.total} />, sub: "across this journal" },
    { label: "Average win", value: <CountUp value={S.avgWin} format={(v) => k(v, currencySymbol)} />, sub: "per winning trade", up: true },
    { label: "Average loss", value: <CountUp value={S.avgLoss} format={(v) => `−${currencySymbol}${fmt(v, 0)}`} />, sub: "per losing trade", up: false },
    { label: "Largest win", value: <CountUp value={S.largestWin} format={(v) => k(v, currencySymbol)} />, sub: "single best trade", up: true },
    { label: "Avg hold time", value: S.holdStr, sub: "median per trade" },
    { label: "Sharpe ratio", value: S.sharpe ? fmt(S.sharpe, 2) : "—", sub: "risk-adjusted return", up: (S.sharpe || 0) > 1 },
    { label: "Win streak", value: S.streak, sub: `best: ${S.bestStreak}`, up: S.streak > 0 },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-5)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "var(--space-3)" }}>
        ...
        <Button variant="primary" icon={Plus} onClick={onLogTrade}>Log a trade</Button>
      </div>
      ...
      <div className="jx-card" style={{ display: "grid", gridTemplateColumns: "minmax(0,1.2fr) minmax(180px,1fr)", gap: "var(--space-5)", alignItems: "center" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "var(--space-2)" }}>
            ...
            <MiniSeg items={["1D", "7D", "21D", "30D"]} value={heroRange} onChange={setHeroRange} />
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
            <span style={{ font: "600 36px/42px var(--jx-font)", letterSpacing: "-1px", color: hero.pnl >= 0 ? "var(--color-text-primary)" : "var(--color-danger-strong)" }}>
          ...
          <div style={{ display: "flex", gap: "var(--space-6)", flexWrap: "wrap" }}>
            {[
              ...
              <span key={l} style={{ display: "flex", flexDirection: "column" }}>
                ...
                <span style={{ font: "var(--text-body-md)", fontWeight: 600 }}>{v}</span>
              </span>
            ))}
          </div>
        </div>
        <AreaChart values={hero.spark.length > 1 ? hero.spark : [0, 0]} height={120} />
      </div>

      {/* ===== Progress cards ===== */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "var(--space-4)" }}>
        <div className="jx-card" style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
          <span style={LABEL}>Win rate</span>
          <Tip content={`${S.winCount} wins ÷ ${S.winCount + S.lossCount} closed trades`}>
            <span style={{ font: "var(--text-stat)", letterSpacing: "-1px", cursor: "help" }}>
        ...
        <div className="jx-card" style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <span style={{ font: "var(--text-caption)", color: "var(--color-text-muted)" }}>{fmt(goalPct, 0)}%</span>
          </div>
          <Tip content={`This month's P&L vs your ${currencySymbol}${fmt(target, 0)} target\n${fmt(goalPct, 0)}% reached`}>
            <span style={{ font: "var(--text-stat)", letterSpacing: "-1px", cursor: "help", color: S.monthPnl >= 0 ? "var(--color-text-primary)" : "var(--color-danger-strong)" }}>
              <CountUp value={S.monthPnl} format={(v) => k(v, currencySymbol)} />
            </span>
          </Tip>
          <Progress pct={goalPct} color="var(--color-primary)" />
          <span style={{ font: "var(--text-caption)", color: "var(--color-text-muted)" }}>
            of {currencySymbol}{fmt(target / 1000, target >= 10000 ? 0 : 1)}k target this month · set in Settings
          </span>
        </div>
        ...
        <div className="jx-card" style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <span style={{ font: "var(--text-caption)", color: "var(--color-text-muted)" }}>target 3.0</span>
          </div>
          <Tip content={S.profitFactor ? `Gross win ${k(S.grossWin, currencySymbol)} ÷ gross loss ${k(-S.grossLoss, currencySymbol)}` : "Needs at least one win and one loss"}>
            <span style={{ font: "var(--text-stat)", letterSpacing: "-1px", cursor: "help" }}>
              {S.profitFactor ? <CountUp value={S.profitFactor} format={(v) => fmt(v, 2)} /> : "—"}
            </span>
          </Tip>
          <Progress pct={S.profitFactor ? (S.profitFactor / 3) * 100 : 0} />
          <span style={{ font: "var(--text-caption)", color: "var(--color-text-muted)" }}>gross win ÷ gross loss</span>
        </div>
        ...
        <div className="jx-card" style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
          ...
          <Progress
            pct={S.discipline.plannedN + S.discipline.unplannedN > 0 ? (S.discipline.plannedN / (S.discipline.plannedN + S.discipline.unplannedN)) * 100 : 0}
            color="var(--color-success)"
          />
          <span style={{ font: "var(--text-caption)", color: "var(--color-text-muted)" }}>
            trades where you followed your plan
            {S.discipline.avgConfidence != null && ` · avg confidence ${fmt(S.discipline.avgConfidence, 1)}★`}
          </span>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: "var(--space-4)", marginTop: "calc(var(--space-3) * -1)" }}>
        {kpis.map((m) => (
          <div key={m.label} className="jx-card" style={{ padding: "var(--space-4) var(--space-5)", display: "flex", flexDirection: "column", gap: 4 }}>
            <span style={LABEL}>{m.label}</span>
            <Tip content={`${m.label} — ${m.sub}`}>
              <span style={{ font: "var(--text-h2)", cursor: "help" }}>{m.value}</span>
            </Tip>
            <span style={{ font: "var(--text-caption)", color: m.up === undefined ? "var(--color-text-muted)" : m.up ? "var(--color-success)" : "var(--color-danger)" }}>
              {m.up !== undefined && (m.up ? "↑ " : "↓ ")}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.6fr) minmax(240px, 1fr)", gap: "var(--space-4)", alignItems: "start" }} className="jx-ov-grid">
        {/* left column */}
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)", minWidth: 0 }}>
          {/* equity growth candles */}
          {(() => {
            const candles = buildEquityCandles(closed, startingBalance, candleTF);
            const equityNow = candles.length ? candles[candles.length - 1].c : startingBalance;
            const growth = startingBalance > 0 ? ((equityNow - startingBalance) / startingBalance) * 100 : null;
            return (
              <div className="jx-card">
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "var(--space-2)" }}>
                  <div>
                    <div style={{ font: "var(--text-body-md)", fontWeight: 600 }}>Equity growth</div>
                    <div style={{ font: "var(--text-h2)" }}>
                      <CountUp value={equityNow} format={(v) => `${currencySymbol}${fmt(v, 0)}`} />
                    </div>
                    <Badge variant={equityNow >= startingBalance ? "success" : "danger"}>
                      {growth != null
                  ...
                  <MiniSeg items={["1D", "1W", "1M", "1Y"]} value={candleTF} onChange={setCandleTF} />
                </div>
                <div style={{ marginTop: "var(--space-3)" }}>
                  <CandleChart key={candleTF} candles={candles} sym={currencySymbol} height={220} />
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", font: "var(--text-caption)", color: "var(--color-text-muted)", marginTop: 6 }}>
                  <span>{candles[0]?.label || ""}</span>
                  <span>each candle = {candleTF === "1D" ? "1 day" : candleTF === "1W" ? "1 week" : candleTF === "1M" ? "1 month" : "1 year"} of your equity</span>
          ...
          <div className="jx-card">
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "var(--space-2)" }}>
              <div>
                <div style={{ font: "var(--text-body-md)", fontWeight: 600 }}>Cumulative P&L</div>
                <div style={{ font: "var(--text-h2)" }}>{k(cumSeries[cumSeries.length - 1] || 0, currencySymbol)} net</div>
                <Badge variant={(cumSeries[cumSeries.length - 1] || 0) >= 0 ? "success" : "danger"}>
                  {inWindow(closed, pnlRange).length} trades in range
                </Badge>
              </div>
              <MiniSeg items={["1D", "1W", "1M", "1Y"]} value={pnlRange} onChange={setPnlRange} />
            </div>
            <div style={{ marginTop: "var(--space-3)" }}>
              <AreaChart values={cumSeries.length > 1 ? cumSeries : [0, 0]} height={170} labels={["Start", "", "", "", "", "Now"]} tips={cumTips.length > 1 ? cumTips : null} />
            </div>
          </div>
          ...
          <div className="jx-card">
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "var(--space-2)" }}>
              <div>
                <div style={{ font: "var(--text-body-md)", fontWeight: 600 }}>Daily P&L</div>
                <div style={{ font: "var(--text-h2)" }}>{k(S.daily.slice(dailyRange === "Week" ? -7 : -30).reduce((s, v) => s + v, 0), currencySymbol)}</div>
                <Badge variant="success">+{S.daily.slice(dailyRange === "Week" ? -7 : -30).filter((v) => v > 0).length} winning days</Badge>
              </div>
              <MiniSeg items={["Week", "Month"]} value={dailyRange} onChange={setDailyRange} />
            </div>
            <div style={{ marginTop: "var(--space-3)" }}>
              <BarChart
                values={S.daily.slice(dailyRange === "Week" ? -7 : -30)}
                tips={S.daily.slice(dailyRange === "Week" ? -7 : -30).map((v, i) => {
                  const labels = S.dailyLabels.slice(dailyRange === "Week" ? -7 : -30);
                  return `${labels[i] || ""}\n${k(v, currencySymbol)}`;
                })}
                height={140}
              />
          ...
          <div className="jx-card">
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "var(--space-2)" }}>
              <div>
                <div style={{ font: "var(--text-body-md)", fontWeight: 600 }}>Win rate trend</div>
                <div style={{ font: "var(--text-h2)" }}>{wrSeries.length ? fmt(wrSeries[wrSeries.length - 1], 1) : fmt(S.winRate, 1)}%</div>
                <Badge variant="neutral">{wrSeries.length} periods</Badge>
              </div>
              <MiniSeg items={["1W", "1M", "3M", "1Y"]} value={wrRange} onChange={setWrRange} />
            </div>
            <div style={{ marginTop: "var(--space-3)" }}>
              <LineChart
                values={wrSeries.length > 1 ? wrSeries : [50, S.winRate || 50]}
                tips={wrSeries.length > 1 ? wrSeries.map((v, i) => `Period ${i + 1}\n${fmt(v, 1)}% win rate`) : null}
                height={140}
              />
            </div>
          </div>
          ...
          <div className="jx-card">
            <div style={{ font: "var(--text-body-md)", fontWeight: 600, marginBottom: "var(--space-3)" }}>P&L by strategy</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {S.stratList.map(([name, v]) => (
                <Tip key={name} content={`${name}\n${v.n} trades · ${v.w} wins (${fmt((v.w / v.n) * 100, 0)}%)\n${k(v.pnl, currencySymbol)} total P&L`} block>
                <div style={{ display: "flex", alignItems: "center", gap: 10, font: "var(--text-small)", flex: 1, cursor: "help" }}>
                  <span style={{ width: 110, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{name}</span>
                  <div style={{ flex: 1, display: "flex" }}>
                    <span style={{ height: 12, borderRadius: 6, width: `${Math.max(6, (Math.abs(v.pnl) / maxStrat) * 100)}%`, background: v.pnl >= 0 ? "var(--color-success)" : "var(--color-danger)", transition: `width 0.8s ${BAR_EASE}` }} />
                  </div>
                  <span style={{ font: "var(--text-caption)", color: "var(--color-text-muted)", width: 70 }}>{fmt((v.w / v.n) * 100, 0)}% win</span>
                  <span style={{ fontWeight: 600, color: v.pnl >= 0 ? "var(--color-success-strong)" : "var(--color-danger-strong)", whiteSpace: "nowrap" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
          <div className="jx-card">
            <div style={{ font: "var(--text-body-md)", fontWeight: 600, marginBottom: "var(--space-3)" }}>Asset allocation</div>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-4)" }}>
              <Donut segments={S.allocList.map(([, v], i) => ({ value: v, color: DONUT_COLORS[i] }))} />
              <div style={{ display: "flex", flexDirection: "column", gap: 8, flex: 1 }}>
                {S.allocList.map(([sym, v], i) => (
                  <Tip key={sym} content={`${sym}: ${currencySymbol}${fmt(v, 0)} traded value\n${fmt((v / allocTotal) * 100, 1)}% of allocation`} block>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, font: "var(--text-small)", flex: 1, cursor: "help" }}>
                      <span style={{ width: 8, height: 8, borderRadius: "50%", background: DONUT_COLORS[i] }} />
                      <span style={{ color: "var(--color-text-secondary)" }}>{sym}</span>
                      <span style={{ marginLeft: "auto", fontWeight: 600 }}>{fmt((v / allocTotal) * 100, 0)}%</span>
                    </div>
                  </Tip>
                ))}
          ...
          <div className="jx-card">
            <div style={{ font: "var(--text-body-md)", fontWeight: 600, marginBottom: "var(--space-3)" }}>P&L by symbol</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div className="jx-card">
            <div style={{ font: "var(--text-body-md)", fontWeight: 600, marginBottom: "var(--space-3)" }}>Plan followed vs not</div>
            {S.discipline.plannedN + S.discipline.unplannedN === 0 ? (
              <span style={{ font: "var(--text-small)", color: "var(--color-text-muted)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
                {[
                  ["Followed plan", S.discipline.plannedN, S.discipline.plannedWr, "var(--color-success)"],
                  ["Went off-plan", S.discipline.unplannedN, S.discipline.unplannedWr, "var(--color-danger)"],
                ].map(([l, n, w, c]) => (
                  <div key={l} style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", font: "var(--text-small)" }}>
                      <span style={{ fontWeight: 600 }}>{l}</span>
                      <span style={{ color: "var(--color-text-muted)" }}>{n} trades{w != null && ` · ${fmt(w, 0)}% win`}</span>
                    </div>
          ...
          <div className="jx-card">
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "var(--space-2)" }}>
              <span style={{ font: "var(--text-body-md)", fontWeight: 600 }}>Volume traded</span>
            </div>
            <div style={{ font: "var(--text-h2)", marginTop: 4 }}>
              {currencySymbol}{totalVolume >= 1e6 ? `${fmt(totalVolume / 1e6, 2)}M` : `${fmt(totalVolume / 1000, 0)}k`}
            </div>
            <Badge variant="neutral">{analyticsRange} window</Badge>
            <div style={{ marginTop: "var(--space-3)" }}>
              <BarChart
                values={S.vols}
                tips={S.vols.map((v, i) => `${S.volLabels[i] || ""}\n${currencySymbol}${fmt(Math.abs(v), 0)} traded`)}
                height={110}
              />
            </div>
          </div>
