...
const CAT_DESC =
  "Trading strategy, risk management, psychology and journaling guides from JournalX — learn to build a measurable edge.";
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:image" content={`${SITE_URL}${featured?.cover || "/assets/JournalX_Banner.png"}`} />
        ...
        <meta name="twitter:image" content={`${SITE_URL}${featured?.cover || "/assets/JournalX_Banner.png"}`} />
          ...
          {featured && cat === "All" && !q && (
            <Link href={`/blog/${featured.slug}`} style={{ textDecoration: "none", color: "inherit" }}>
              <motion.div
                ...
                style={{ padding: 0, overflow: "hidden", display: "grid", gridTemplateColumns: "minmax(0,1fr) 1.2fr", cursor: "pointer" }}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={featured.cover} alt={featured.title} style={{ width: "100%", height: "100%", minHeight: 240, objectFit: "cover" }} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "var(--space-4)" }}>
            {list.map((p, i) => (
              <Link key={p.slug} href={`/blog/${p.slug}`} style={{ textDecoration: "none", color: "inherit" }}>
                ...
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={p.cover} alt={p.title} style={{ width: "100%", height: 150, objectFit: "cover" }} />
                  <div style={{ padding: "var(--space-4)", display: "flex", flexDirection: "column", gap: 6, flex: 1 }}>
                    <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span style={{ font: "var(--text-caption)", color: "var(--color-text-muted)", display: "flex", alignItems: "center", gap: 4 }}>
