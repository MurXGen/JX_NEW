...
export default function VolumeChart({ dailyData }) {
  ...
  const weekData = useMemo(() => {
    ...
    return days.map((d, i) => {
      ...
      dayTrades.forEach((t) => {
        if (t.longVolume) longVolume += t.longVolume;
        if (t.shortVolume) shortVolume += t.shortVolume;
      });
  ...
  return (
    <div className="chart-container daily-volume-chart-container">
      ...
      <ResponsiveContainer width="100%" height={250}>
        ...
        >
          <defs>
            <linearGradient id="volLong" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#22C55E" stopOpacity={0.9} />
              <stop offset="100%" stopColor="#22C55E80" stopOpacity={0.5} />
            </linearGradient>
            <linearGradient id="volShort" x1="0" y1="0" x2="0" y2="1">
          ...
          >
            <LabelList
              ...
              position="top"
              className="bar-label"
              formatter={(val) => (val === maxVolume ? formatNumber(val) : "")}
            />
          </Bar>
          ...
          >
            <LabelList
              ...
              className="bar-label"
              formatter={(val) =>
                val === maxShortVolume ? formatNumber(val) : ""
              }
            />
          </Bar>
        </BarChart>
